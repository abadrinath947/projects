...
public class ExprUtils {
	 ...
	 *
	 *	Algorithm: 
	 * 	1. Create new ArrayList and an operator Stack. Obtain a tokenized
	 * 	expression and store it in a List. 
	 * 	2. Traverse the tokenized expression and get the individual token.
	 * 	3. If it is a number, add it to the result ArrayList; if it is a
	 * 	left parenthesis, add it to the operatorStack. 
	 * 	4. If it is a right parenthesis, pop every value till the left
	 * 	paranthesis, while adding it to the result ArrayList.
	 * 	5. If it is another operator, simply pop from from the operator
	 * 	Stack into the result and add it to the stack.
	 * 	6. After the main while, empty the operatorStack and add it to
	...
	public List<String> toPostfix(String expr) {
		...
		// get a tokenized list of the expression
		...
		while (c < expression.size()){
			...
			// if it is a right paren
				...
				while (!operatorStack.peek().equals("("))
					// if it is a left paren add the last value of op.
					// stack to the result
					result.add(operatorStack.pop());
				operatorStack.pop();	
			}
			// otherwise add it to the operatorStack
			else {
				while (!operatorStack.isEmpty() &&
					   !(token.equals("(")) &&
					   hasPrecedence(token, operatorStack.peek()))
					// add the last value of op. stack to the result
					result.add(operatorStack.pop());
		...
		// add the operators at the end of the ArrayList
		while (!operatorStack.isEmpty()){
			result.add(operatorStack.pop());
		}
