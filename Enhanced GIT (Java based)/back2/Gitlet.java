...
public class Gitlet {
    ...
    public Gitlet(String[] args) {
        if (args[0].equals("init")) {
            ...
            System.out.println(this._currentHeadPointer);
            generateBlobs();
    ...
    private boolean unStage(String fileName) {
        for (String blob: this._stagedBlobs.keySet()) {
            if (searchBlobSHA1(blob).getIdentity().equals(fileName) &&
                this._stagedBlobs.get(blob).equals(this._currentHeadPointer.getCurrentCommit())) {
    ...
    private void writeCommits() {
        ...
        } REASON FOR BUG: NOT UPDATING CURRENT COMMIT PTRS, but this wont work with branches */
        writeCommitsRecursive(this._commitSHA1Tree);
    }
    ...
    private void mapCommitPointers(Commit commit) {
        if (this._currentCommitPointers == null) {
            ...
            for (SHA1Tree<String> i: this._currentCommitPointers.keySet()) {
                if (searchCommitSHA1(i.getSHA1()).equals(commit.getParent())) {
                    ...
                    this._commitSHA1Tree.addCommit(searchCommitSHA1(commit.getParent()).getSHA1(), commit.getSHA1());
