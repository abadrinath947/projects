...
class Place {
    ...
    static PlaceList[][][] successorCells(int width, int height) {
        ...
        for (int i = 0; i < M.length; i++) {
            for (int j = 0; j < M[i].length; j++) {
                M[i][j] = findQueenMoves(M, M[i][j], i, j);
            }
        }
        return M;
    }

    /** Returns a PlaceList array from an empty initialization DIRS
     *  such that all possible successors for a Sq at X, Y in a
     *  particular direction k are accessible through M[X][Y][k]. */
    static PlaceList[] findQueenMoves(PlaceList[][][] M,
                                      PlaceList[] dirs, int x, int y) {
        for (int i = 0; i < dirs.length; i++) {
            dirs[i] = new PlaceList();
        }
        for (int i = 0; i < M.length; i++) {
            for (int j = 0; j < M[i].length; j++) {
                if (dirOf(x, y, i, j) != 0) {
                    Place temp = pl(i, j);
                    dirs[dirOf(x, y, i, j)].add(temp);
                    dirs[0].add(temp);
                }
            }
        }
    ...
    /** Returns x coordinate of Place this. */
    int getX() {
        return x;
    }

    /** Returns y coordinate of Place this. */
    int getY() {
        return y;
    }
