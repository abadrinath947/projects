    ...
    private int makeForwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                for (Place possiblePlace: sq.successors()) {
                    Sq possibleSuccessor = model.get(possiblePlace);
                    if (sq.connectable(possibleSuccessor)) {
                        found = possibleSuccessor;
                        nFound++;
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                for (Place possiblePlace: sq.predecessors()) {
                    Sq possiblePredecessor = model.get(possiblePlace);
                    if (possiblePredecessor.connectable(sq)) {
                        found = possiblePredecessor;
                        nFound++;
                        if (sq.sequenceNum() != 0 && found.sequenceNum() != 0) {
                            nFound = 1;
                            break;
        ...
        do {
            ...
            for (Sq sq : model) {
                if (sq.hasFixedNum() && sq.sequenceNum() != 1
                    ...
                    if (model.solved()) {
                        changed = true;
                    } else if (sq.sequenceNum() == 0 || sq.sequenceNum() == n) {
                        sq.setFixedNum(n);
                    }
                }
            }
